...
# Boxplots for all counts looks crazy, but apparently this is common in RNAseq data and is accounted for by DESeq2 by a size factor approach (can back up later with a source)
...
keep <- rowSums(counts_matched_T_NM >= 10) >= 57
...
#######
##### 2025/02/20 
## I want to try PCA here again to make sure the T vs NM separation is still good after previous few steps

colz3 <- as.numeric(as.factor(groups)) # Get color values from group
plotMDS(voom_data$E,
        gene.selection = "common",
        main = "PCA for TCGA-LUAD expression after outlier removal",
        col = colz3,
...
## Yes the separation looks great
...
### Replace ensembl IDs with gene names. [[[FIXED MAJOR LABELLING ERROR 2025/02/21]]]
...
# Save output
write.table(TCGA_LUAD_limma_DEG, "../2_Outputs/4_Tumor_DEGs/TCGA_LUAD_limma_DEG_CSFSNS_20250221.txt", sep = '\t')

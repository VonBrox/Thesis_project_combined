...
## 2.1.2 Formatting as tumor-normal pairs, in this case NOT removing never-smoker samples, as a comparison
```{r}
...
# Save output
write.table(TCGA_LUAD_limma_DEG, "../2_Outputs/4_Tumor_DEGs/TCGA_LUAD_limma_DEG_CSFSNS_20250131.txt", sep = '\t')
...
### 2025/01/31: Comparing what I got this way with what I got after removing never smokers, both times using limma.
```{r}
library(ggplot2)

TCGA_LUAD_limma_DEG_CSFSNS <- TCGA_LUAD_limma_DEG
TCGA_LUAD_limma_DEG_CSFS <- read.table("../2_Outputs/4_Tumor_DEGs/TCGA_LUAD_limma_DEG_CSFS_20250129.txt")

TCGA_LUAD_limma_DEG_comparing_CSFS_CSFSNS <- full_join(TCGA_LUAD_limma_DEG_CSFSNS, TCGA_LUAD_limma_DEG_CSFS, by = "Gene", suffix = c("_CSFSNS", "_CSFS"))

# basic scatterplot for FDR > 0.05 genes
ggplot(TCGA_LUAD_limma_DEG_comparing_CSFS_CSFSNS %>% filter(FDR_CSFSNS >= 0.05) %>% filter (FDR_CSFS >= 0.05), aes(x=log2FC_CSFSNS, y=log2FC_CSFS)) + 
    geom_point()
## Wowwee there is not a lot of relationship between signs even. What about lengths?

nrow(TCGA_LUAD_limma_DEG_comparing_CSFS_CSFSNS %>% filter(FDR_CSFSNS >= 0.05))
nrow(TCGA_LUAD_limma_DEG_comparing_CSFS_CSFSNS %>% filter (FDR_CSFS >= 0.05))
nrow(TCGA_LUAD_limma_DEG_comparing_CSFS_CSFSNS %>% filter(FDR_CSFSNS >= 0.05) %>% filter (FDR_CSFS >= 0.05))
nrow(TCGA_LUAD_limma_DEG_comparing_CSFS_CSFSNS %>% filter(FDR_CSFSNS >= 0.05) %>% filter (FDR_CSFS >= 0.05) %>% filter(sign(log2FC_CSFSNS)==sign(log2FC_CSFS)))
## Wowwee not even a lot of overlap. It is interesting that removing never smokers makes such a big difference actually!

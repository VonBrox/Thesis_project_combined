...
### Plotting the boxplots of genes of interest
```{r}
...
expr_current_smokers_curr_gene <- GSE4302_CS_NS_expr_matrix_genes_of_interest[GSE4302_CS_NS_expr_matrix_genes_of_interest$gene==curr_gene_of_interest, GSE4302_current_smoker_indexes]

expr_never_smokers_curr_gene <- GSE4302_CS_NS_expr_matrix_genes_of_interest[GSE4302_CS_NS_expr_matrix_genes_of_interest$gene==curr_gene_of_interest, GSE4302_never_smoker_indexes]

# Combine into a long format dataframe
expr_long <- data.frame(
  Gene = curr_gene_of_interest,
  current_smokers = as.vector(expr_current_smokers_curr_gene[,-1]),
  never_smokers   = as.vector(expr_never_smokers_curr_gene[,-1])
) %>%
...
ggplot(expr_long, aes(x = Group, y = Expression, fill = Group)) +
  ...
  theme_minimal() +
  labs(title = "GSE4302: Current Smokers vs Never Smokers",
...
## Turning above into an index-based process
```{r}
...
# Set current gene of interest (make a loop eventually)
i<-2
curr_gene <- genes_of_interest[i]

# Set current dataset index of interest (make loop eventually)
j<-1
...
for (i in 1:length(genes_of_interest)){
  ...
  curr_gene <- genes_of_interest[i] # Loop through current gene index of interest

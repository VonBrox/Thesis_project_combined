...
linked_genes <- read.table('../2_Outputs/6_Linked_genes/A1_TE_TM_linked_genes_0.01_0.25_0.001_0.75_0.01_0.25_20250101.txt', header = TRUE)

# Read in the persistent linked genes
persistent_linked_genes <- read.table('../2_Outputs/6_Linked_genes/A1_TE_TM_A2_persistent_linked_genes_0.01_0.25_0.001_0.75_0.01_0.25_20250101.txt', header = TRUE)
...
# Extract the "linked genes" and "persistent linked genes" and check their rank order
linked_CS_NS_ranked_genes <- CS_NS_ranked_genes[CS_NS_ranked_genes$Gene.symbol %in% linked_genes$Gene,]
hist(as.numeric(linked_CS_NS_ranked_genes$rank_order), main = "Linked genes", xlab = "Rank")

persistent_linked_CS_NS_ranked_genes <- CS_NS_ranked_genes[CS_NS_ranked_genes$Gene.symbol %in% persistent_linked_genes$Gene,]
hist(as.numeric(persistent_linked_CS_NS_ranked_genes$rank_order), main = "Persistent linked genes", xlab = "Rank")
...
hist(CS_NS_ranked_genes$num_studies, 
     xlim = c(0.5, 5.5),  # Adjust x-axis limits slightly
     ylim = c(0,6000),
     breaks = seq(0.5, 5.5, by = 1),  # Shift breaks by 0.5
...
dataframe_DEGs_list <- list(GSE4302 = GSE4302_CS_NS_GEO2R_limma_sig, GSE63127 = GSE63127_CS_NS_GEO2R_limma_sig, GSE19027 = GSE19027_CS_NS_GEO2R_limma_sig, GSE20257 = GSE20257_CS_NS_GEO2R_limma_sig, GSE7895 = GSE7895_CS_NS_GEO2R_limma_sig)
...
## Put the dataframes in a list
dataframe_list <- list(GSE4302 = GSE4302_CS_NS_mean_SD, GSE7895 = GSE7895_CS_NS_mean_SD, GSE19027 = GSE19027_CS_NS_mean_SD, GSE14633 = GSE14633_CS_NS_mean_SD, GSE20257 = GSE20257_CS_NS_mean_SD, GSE63127 = GSE63127_CS_NS_mean_SD)
...
2024/11/19 and 2024/12/05: So 6 of those 27 persistent linked genes have significant cumulative SMD across the airway datasets (p < 0.05)
...
# Make a list of the genes of interest (persistent linked genes with significant cumulative SMD) for the boxplots
genes_of_interest <- c("CA12", "CCDC81", "CTNNAL1", "EFEMP1", "LMO2", "TNS1")

### GSE14633 ###
GSE14633_expr_matrix_gene <- cbind(gene = GSE14633_gene_symbol$x, GSE14633_expr_matrix)

GSE14633_gene_probe_of_interest <- GSE14633_CS_NS_GEO2R_limma_sig %>%
  filter(Gene.symbol %in% genes_of_interest) %>%
  select(Gene.symbol, ID)

GSE14633_CS_NS_expr_matrix_genes_of_interest <- GSE14633_expr_matrix_gene %>%
  filter(rownames(.) %in% GSE14633_gene_probe_of_interest$ID)
...
## Put the dataframes in a list
df_list_goi <- list(GSE14633 = GSE14633_CS_NS_expr_matrix_genes_of_interest, GSE4302 = GSE4302_CS_NS_expr_matrix_genes_of_interest, GSE7895 = GSE7895_CS_NS_expr_matrix_genes_of_interest, GSE19027 = GSE19027_CS_NS_expr_matrix_genes_of_interest, GSE20257 = GSE20257_CS_NS_expr_matrix_genes_of_interest, GSE63127 = GSE63127_CS_NS_expr_matrix_genes_of_interest)

## Put the indexes into lists
current_smoker_indexes <- list(GSE14633_csi = GSE14633_current_smoker_indexes, GSE4302_csi = GSE4302_current_smoker_indexes, GSE7895_csi = GSE7895_current_smoker_indexes, GSE19027_csi = GSE19027_current_smoker_indexes, GSE20257_csi = GSE20257_current_smoker_indexes, GSE63127_csi = GSE63127_current_smoker_indexes)

never_smoker_indexes <- list(GSE14633_nsi = GSE14633_never_smoker_indexes, GSE4302_nsi = GSE4302_never_smoker_indexes, GSE7895_nsi = GSE7895_never_smoker_indexes, GSE19027_nsi = GSE19027_never_smoker_indexes, GSE20257_nsi = GSE20257_never_smoker_indexes, GSE63127_nsi = GSE63127_never_smoker_indexes)

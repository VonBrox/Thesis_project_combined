...
According to this TCGA publication:
...
for (gene in genes_of_interest){
  ...
  # Group assignment
  former_smokers$group <- ifelse(t(gene_of_interest_FS_expression_values) <= low_threshold, "Low",
                          ifelse(t(gene_of_interest_FS_expression_values) >= high_threshold, "High", NA))
  ...
  ggsp <- ggsurvplot(fit, data = former_smokers_high_low_goi, pval = TRUE, title = gene_of_interest)
  print(ggsp)
}

## Significant genes found: CCDC81, SERPINB5, S100P

```

Nice, the next thing to do would be to check if these 3 genes (CCDC81, SERPINB5, S100P) which predict survival in former smokers, also just predict survival in lung cancer or cancer in general. If they do, it might diminish the relevance of the finding, or it might back it up, depending on how you look at it...

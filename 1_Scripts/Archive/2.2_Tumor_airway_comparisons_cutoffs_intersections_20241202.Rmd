...
2024/12/02: Now unlike the way I did it in the big combined script, here I am going to try out doing more stringent FDR cutoffs instead of log2FC cutoffs. I may also change the methylation list input, in fact I probably should (either use limma with cbioportal or confirm results from the probe-wise version).
...
nrow(DGE_LUAD_T_NM_DESeq2_sig[DGE_LUAD_T_NM_DESeq2_sig$FDR<FDR_TE
                              & abs(DGE_LUAD_T_NM_DESeq2_sig$log2FC)>log2FC_cutoff_TE,])

hist(DGE_LUAD_T_NM_DESeq2_sig[DGE_LUAD_T_NM_DESeq2_sig$FDR<FDR_TE
...
nrow(DMeth_LUAD_T_NM_hm450_sig[DMeth_LUAD_T_NM_hm450_sig$FDR<FDR_cutoff_TM
                               & abs(DMeth_LUAD_T_NM_hm450_sig$log2foldChange)>log2FC_cutoff_TM,])

hist(DMeth_LUAD_T_NM_hm450_sig[DMeth_LUAD_T_NM_hm450_sig$FDR<FDR_cutoff_TM
                               & abs(DMeth_LUAD_T_NM_hm450_sig$log2foldChange)>log2FC_cutoff_TM,]$log2foldChange, breaks = 100)
...
DMeth_LUAD_T_NM_hm450_sig_cutoff <- DMeth_LUAD_T_NM_hm450_sig %>%
  filter(abs(log2foldChange)>log2FC_cutoff_TM) %>%
...
Below is using log2FC cutoffs, above is using more stringent FDR cutoffs as well as log2FC cutoffs

###

# II. Using log2FC cutoffs
...
2024/11/27: Here is a very significant interaction network I got when running using the DESeq2 TE with no cutoffs:
https://version-12-0.string-db.org/cgi/network?networkId=baZv6RvUQLCS
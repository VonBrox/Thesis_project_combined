...
##exprs(gset) <- normalizeBetweenArrays(exprs(gset)) # quantile normalization: no longer doing this
...
# 4. Comparing and filtering A1, TE, and TM to generate "linked genes" list
...
## Notes on results after filtering:
# 0, 0, 0 => 950 genes
# 0.2, 1, 0.3 => 143 genes
# 0.5, 2, 0.5 => 14 genes
...
### 4.2.1 Intersection of A1 and TE DEGs
```{r}
## Merge the lists
...
### 4.2.2 Intersection of A1/TE DEGs with TM DMGs
```{r}
## Merge the lists
A1_TE_TM_merged_DEGs_DMGs <- A1_TE_merged_DEGs %>%
...
nrow(A1_TE_TM_merged_DEGs_DMGs)
...
### 4.2.3 Visualizing Spearman correlations of log2FC values
```{r}
## Visualizing Spearman correlations
library(GGally)
ggpairs(A1_TE_TM_merged_DEGs_DMGs[, c("log2FC_A1", "log2FC_TE", "log2FC_TM")],
...
### 4.2.4 Visualizing multiple linear regression of the log2FC values
```{r}
mlr_airway_model <- lm(log2FC_A1 ~ log2FC_TE + log2FC_TM, data = A1_TE_TM_merged_DEGs_DMGs)
summary(mlr_airway_model)


library(scatterplot3d)
s3d <- scatterplot3d(x = A1_TE_TM_merged_DEGs_DMGs$log2FC_A1, 
              y = A1_TE_TM_merged_DEGs_DMGs$log2FC_TE, 
              z = A1_TE_TM_merged_DEGs_DMGs$log2FC_TM,
              ...
              color = "steelblue",
              labels = A1_TE_TM_merged_DEGs_DMGs$Gene,
              type = "h",
              #highlight.3d = TRUE, 
              angle = 60
              )
...
### 5.2.1 Initial checks (histogram, boxplot, PCA)
```{r}
...
#exprs(gset) <- normalizeBetweenArrays(exprs(gset)) # electing not to do quantile normalization for now
...
### 5.2.2 Investigating source of variation
...
#### 5.2.3 Plot PCA using other phenotypic data
```{r}
...
## 5.4 Select "persistent" DEGs, and basic filter (keep lower FDR of duplicates, apply FDR < 0.05 cutoff)
```{r}
library(dplyr)
library(VennDiagram)
...
GSE7895_persistent <- tT_persistent[,c("ID", "Gene.symbol", "CS_vs_NS", "FS_vs_NS", "CS_vs_FS", "adj.P.Val")]

```

2024/11/12: There are only 77 genes here when I got 116 the first time I ran it. The only difference was quantile normalization, so I should test whether that did it.

For now though, I want to check whether these genes now stratify the groups.

## 5.5 Extra checks (PCA for stratification)
```{r}
...
# Might be good to check on the packyears and TSQ here as well?
